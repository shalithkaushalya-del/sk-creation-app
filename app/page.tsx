    ...
    <div className="min-h-screen bg-[#111113] text-white p-6 md:p-10">
      <header className="flex justify-between items-center mb-10 border-b border-gray-800 pb-6">
        ...
        <div className='flex gap-2 bg-gray-900 p-1 rounded-full border border-gray-800 flex-wrap justify-center'>
          ...
          <Link href="/" className="bg-blue-600 text-white px-4 py-2 rounded-full text-xs font-bold hover:bg-blue-700 transition">View Site</Link>
        </div>
      </header>
      ...
      <form onSubmit={handleSubmit} className="bg-[#1c1c1e] p-6 rounded-2xl flex flex-col gap-4 mb-10 border border-gray-800">
        ...
        <input type="text" placeholder="Name" value={name} onChange={(e) => setName(e.target.value)} required className="bg-black p-3 rounded-lg outline-none border border-gray-800 focus:border-blue-500 transition" />
        <select value={category} onChange={handleCategoryChange} className="bg-black p-3 rounded-lg outline-none border border-gray-800 focus:border-blue-500 transition">
        ...
        <input type="text" value={itemCode} readOnly className="bg-gray-900 p-3 rounded-lg cursor-not-allowed border border-gray-800" />
        <input type="text" placeholder="Price" value={price} onChange={(e) => setPrice(e.target.value)} required className="bg-black p-3 rounded-lg border border-gray-800 focus:border-blue-500 transition" />
        <input type="file" onChange={(e) => setImage(e.target.files?.[0] || null)} className="bg-black p-3 rounded-lg border border-gray-800" />
        <button type="submit" disabled={uploading} className="bg-blue-600 hover:bg-blue-700 transition p-3 rounded-lg font-bold">{uploading ? 'Processing...' : 'Save Product'}</button>
      </form>
        ...
        {products.map((item) => (
          <div key={item.id} className="bg-[#1c1c1e] p-4 rounded-xl border border-gray-800">
            ...
            <div className="flex gap-2 mt-4">
              <button onClick={() => handleEdit(item)} className="flex-1 bg-gray-700 hover:bg-gray-600 transition py-2 rounded-lg text-sm">Edit</button>
              <button onClick={() => handleDelete(item.id)} className="flex-1 bg-red-900/80 hover:bg-red-800 transition py-2 rounded-lg text-sm">Delete</button>
            </div>
          </div>
        ))}
      </div>
