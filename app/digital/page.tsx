  ...
  useEffect(() => {
    async function fetchProducts() {
      const { data, error } = await supabase
        .from('products')
        .select('*')
        // 👇 මෙතන 'Digital' වෙනුවට 'SKD' කියලා හැදුවා
        .eq('category', 'SKD') 
        .order('id', { ascending: false }); // අලුත්ම ඒවා උඩින් පේන්න දැම්මා
  ...
  return (
    <main className="min-h-screen bg-[#111113] text-white p-6 md:p-12 relative">
      <header className="flex justify-between items-center mb-10">
        <h1 className="text-3xl font-black text-indigo-500">SK DIGITAL STORE</h1>
        ...
        <div className="flex items-center gap-6">
          ...
          <div onClick={toggleCart} className="relative bg-gray-800 p-3 rounded-full cursor-pointer hover:bg-gray-700 transition">
            🛒
            {cart.length > 0 && (
              <span className="absolute -top-2 -right-2 bg-indigo-500 text-white text-xs font-bold w-6 h-6 flex items-center justify-center rounded-full">
                {cart.length}
              </span>
            )}
          </div>
      ...
      {products.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {products.map((item) => (
            <div key={item.id} className="bg-[#1c1c1e] p-4 rounded-3xl border border-gray-800 hover:border-indigo-500 transition">
              <img 
              ...
              <button 
                ...
                className="w-full bg-indigo-600 py-2 rounded-xl font-bold hover:bg-indigo-700 transition"
              >
                Add to Cart
              </button>
            </div>
