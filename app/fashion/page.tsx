  ...
  useEffect(() => {
    async function fetchProducts() {
      const { data, error } = await supabase
        .from('products')
        .select('*')
        // 👇 මෙතන 'Fashion' වෙනුවට 'SKF' කියලා හැදුවා
        .eq('category', 'SKF') 
  ...
  return (
    <main className="min-h-screen bg-[#111113] text-white p-6 md:p-12 relative">
      <header className="flex justify-between items-center mb-10">
        <h1 className="text-3xl font-black text-red-500">SK FASHION STORE</h1>
      ...
      {products.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {products.map((item) => (
            <div key={item.id} className="bg-[#1c1c1e] p-4 rounded-3xl border border-gray-800 hover:border-red-500 transition">
              <img 
              ...
              <p className="text-red-400 font-bold mb-4">{item.price}</p>
              ...
              <button 
                ...
                className="w-full bg-red-600 py-2 rounded-xl font-bold hover:bg-red-700 transition"
              >
                Add to Cart
              </button>
            </div>
